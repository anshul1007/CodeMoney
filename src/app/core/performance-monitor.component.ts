...
export class PerformanceMonitorComponent {
  ...
  constructor() {
    ...
    this.showMonitor.set(
      this.isDevelopment || this.performanceService.hasPerformanceIssues(),
    );
        ...
        navigator.clipboard
          ...
          .then(() => {
            ...
            this.showNotification(
              'Performance report copied to clipboard!',
              'success',
            );
          })
          .catch(() => {
            console.warn(
              '📋 Could not copy to clipboard, logging report instead',
            );
  ...
  private getPerformanceRecommendations(metrics: any, score: number): string[] {
    ...
    if (recommendations.length === 0) {
      recommendations.push(
        '✅ Performance looks good! Consider monitoring trends over time.',
      );
    }
